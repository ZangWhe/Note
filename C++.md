...
### 	14 strlen,sizeof,length,size之间的区别
...
1. **预处理阶段**：
   首先，预处理器会对源代码进行处理，主要包括以下几个步骤：

- 处理包含指令：将包含指令（#include）中引入的头文件插入到源代码中。
- 宏替换：将源代码中的宏调用替换为宏的展开式。
- 条件编译：根据条件编译指令（如#ifdef、#ifndef、#if、#else、#elif和#endif），选择性地编译源代码。

预处理后的代码被称为预处理文件，通常以“.i”或“.ii”为后缀名。

2. **编译阶段**：
   经过预处理后，编译器将预处理文件转换为汇编语言代码，主要包括以下几个步骤：

- 词法分析：将源代码分解成一个个的标记（Tokens）。
- 语法分析：将标记组成语法树，检查语法错误。
- 语义分析：对语法树进行类型检查，检查语义错误。
- 代码生成：将语法树转换为汇编语言代码。

编译后的代码被称为汇编文件，通常以“.s”或“.asm”为后缀名。

3. **汇编阶段**：
   接下来，汇编器将汇编文件转换为机器码（目标文件），主要包括以下几个步骤：

- 词法分析：将汇编文件拆解成一系列的汇编指令。
- 语法分析：将汇编指令组织成语法树，检查语法错误。
- 代码生成：将语法树转换为目标文件，其中包含机器码和符号表。

汇编后的代码被称为目标文件，通常以“.o”、“obj”或“.out”为后缀名。

4. **链接阶段**：
   最后，链接器将目标文件和其他可能需要的库文件链接起来，生成可执行文件，主要包括以下几个步骤：

- 符号解析：将目标文件中引用的符号与其他目标文件或库文件中定义的符号进行匹配。
- 地址重定位：将目标文件中的相对地址转换为绝对地址。
- 符号解析和重定位主要是针对全局变量和函数。
- 代码合并：将链接后的机器码整合到单个文件中。
- 生成可执行文件：将链接得到的机器码生成可执行文件，即可在操作系统中运行的二进制文件。

链接后的代码就是最终的可执行文件，通常以无后缀名或“.exe”为后缀名。

总之，编译过程包括预处理、编译、汇编和链接四个阶段。每个阶段都有不同的功能和处理步骤，并最终生成一个可在操作系统中执行的可执行文件。
